...
@section('content')
    <div class="mb-5">
        <ul class="m-0 p-0 list-none">
            ...
            <li class="inline-block relative text-sm text-slate-500 font-Inter dark:text-white">Edit Gateway</li>
        </ul>
    </div>
            ...
            <div class="card col-span-12 lg:col-span-12">
                <div class="card-body flex flex-col p-6">
                    <header class="flex items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                        <div class="flex-1">
                            <div class="card-title text-slate-900 dark:text-white">Edit Gateway</div>
                        </div>
                    </header>
                    <div class="card-text h-full">
                        <form class="space-y-4" method="POST" action="{{ route('admin.gateways.update', $gateway) }}"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-7">
                                <div class="input-area">
                                    ...
                                    <select name="type" class="form-control" id="gw_type">
                                        @php($t = old('type', $gateway->type))
                                        <option value="deposit" @selected($t === 'deposit')>Deposit</option>
                                        <option value="withdraw" @selected($t === 'withdraw')>Withdraw</option>
                                    </select>
                                </div>
                                <div class="input-area">
                                    <label class="form-label">Gateway Method</label>
                                    <input type="text" name="method" value="{{ old('method', $gateway->method) }}"
                                        class="form-control" placeholder="e.g., bKash, Nagad" required>
                                </div>
                                <div class="input-area" id="account_wrap">
                                    <label class="form-label">Account Number (for Deposit)</label>
                                    <input type="text" name="account_number"
                                        value="{{ old('account_number', $gateway->account_number) }}" class="form-control"
                                ...
                                <div class="flex items-center space-x-2">
                                    <label
                                        ...
                                        <input type="checkbox" name="status" value="active" class="sr-only peer"
                                            {{ old('status', $gateway->status) === 'active' ? 'checked' : '' }}>
                                        <span
